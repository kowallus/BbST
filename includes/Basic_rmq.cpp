...
namespace rmqrmm {
	...
	long long int getNumLI64(long long int *A, ulong ini, uint len){
		ulong i=ini>>BW64, j=ini-(i<<BW64);
		long long int result = (A[i] << j) >> (W64-len);
	...
	void setNum64(ulong *A, ulong ini, uint len, ulong x) {
		...
		if ((j+len)>W64){
			...
			myMask = ~0ull >> (j+len-W64);
			A[i+1] = (A[i+1] & myMask) | (x << (WW64-j-len));
		}else{
			ulong myMask = (~0ull >> j) ^ (~0ull << (W64-j-len)); // XOR: 1^1=0^0=0; 0^1=1^0=1
	...
	void setNumLI64(long long int *A, ulong ini, uint len, long long int x) {
		ulong i=ini>>BW64, j=ini-(i<<BW64);
