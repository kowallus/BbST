...
RMQRMM64::RMQRMM64(long long int *A, ulong len) {
	init(len);
...
void RMQRMM64::createTables(){
	...
	long long int semiSumBlock, Min, auxMin;
...
long long int RMQRMM64::sumAtPos(long long int pos){
	ulong rb, q;
	ulong blk = pos>>BBLK;
	long long int j, l, sum=getNum64(TSumB, blk*lg_SumB, lg_SumB)<<1;
...
// give the excess from 0 to pos
long long int RMQRMM64::sumAtBlock(long long int blk){
...
long long int RMQRMM64::computeSumOfNode(ulong node, ulong dist){
	long long int sum = 0;
...
void RMQRMM64::search_min_block(ulong x1, ulong x2, long long int *min, long long int *curSum, ulong *position){
	int Min, sum, auxMin;
	uint rest, q;
...
ulong RMQRMM64::rmqi_rmm(ulong x1, ulong x2, long long int *min, long long int *currSum, ulong posMin){
	ulong leafL, lLeaf, aLeaf, node, nodeMin, posM, g, dNode, dist = 1;
	long long int minT, sumMin;
	bool isNode = false;
	bool isLeaf = false;
	...
	long long int Min = *min, sum = *currSum;
	...
	if (sumMin > (long long int)getNum64(TMinB, nodeMin*lg_MinB, lg_MinB))
...
void RMQRMM64::test_rmqi(){
	...
	while(i<nP){
		...
		while(j<nP){
			...
			if (j<nP){
				for (sum=0, Min=0, posMin=i, k=j; k>=(long long int)i; k--){
					if(readBit64(P, k)){
						sum++;
						if (Min < sum){
							Min = sum;
